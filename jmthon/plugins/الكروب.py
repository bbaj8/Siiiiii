...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**☭ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @R125R**")
...
async def tban(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("☭ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @R125E")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    cattime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "☭ ⦙   هذه قناة إختبار أُنشئت بإستعمال العرب دايفل"
    else:
        descript = "☭ ⦙   هذه المجموعه إختبار أُنشئت بإستعمال دايفل العرب"
